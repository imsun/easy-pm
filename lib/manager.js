...
const { resolveHome, getHash, getAppDir, getFreePort } = require('./utils')
	...
	return fs.readFile(configPath, 'utf8')
		.then(configStr => {
			...
			return Promise.all(config.apps.map(app => {
				...
				const configPathHash = getHash(configPath)
				let script
				let args
				...
				switch (type) {
					case 'node':
						...
						break
					case 'static':
						const rootFlag = app.root ? `-r ${app.root}` : ''
						const notFoundFlag = app['404'] ? `-n ${app['404']}` : ''
						const fallbackFlag = app.fallback ? `-f ${app.fallback}` : ''
						script = staticServerScript
						args = `${rootFlag} ${notFoundFlag} ${fallbackFlag}`
						break
					default:
